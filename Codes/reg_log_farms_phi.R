...
Reg_fin =  glm(DIFF ~ ., data = farms, family = binomial(link = "logit"))

score_1 = filter(data.frame(Reg_fin$fitted.values, farms$DIFF), farms$DIFF == "saine")  #vrai score, ceux qui sont 1
score_2 = filter(data.frame(Reg_fin$fitted.values, farms$DIFF), farms$DIFF == "défaillante")  #vrai score, ceux qui sont 0


hgB1 = hist(score_1$Reg_fin.fitted.values, breaks=20, plot=F) #histogramme des scores des vraies fermes saines
hgB2 = hist(score_2$Reg_fin.fitted.values, breaks=20, plot=F) #histogramme des scores des vraies fermes défaillantes
...
lines(density(score_2$Reg_fin.fitted.values), lwd=1.5)
